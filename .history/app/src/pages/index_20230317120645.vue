...
const initMap = (position: GeolocationPosition) => {
  ...
  const numRows = 71 // 71
  const numCols = 462 // 462
  const cellWidth = 0.001
  const cellHeight = 0.001
  const totalWidth = cellWidth * numCols
  const totalHeight = cellHeight * numRows
  const startLat = position.coords.latitude - totalHeight / 2
  const startLng = position.coords.longitude - totalWidth / 2
